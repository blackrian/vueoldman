...
//            组件核心，计算当前父级需要进行的偏移，每次要遍历节点，应该直接存储节点，提高性能
            currentWidth:{
                get(){
                    let poswidth=0;
                    let $slider;
        ...
        methods:{
            ...
            swipeEnd(e){
                ...
                let now=new Date().getTime(),
                        deltaTime=now-this.sliderinit.start.t,
                        deltaX=this.sliderinit.end.x-this.sliderinit.start.x,
                        deltaY=this.sliderinit.end.y-this.sliderinit.start.y;
                //自动滚动重启
                if(this.sliderinit.autoplay){
                    let that=this;
                    setTimeout(()=>{
